...
class Pushbullet():
    ...
    def file(self, filepath, deviceid=None, apikey=None, body=None):
        ...
        self._upload_and_push_file(self, filepath=filepath, body=body, deviceid=deviceid, apikey=apikey)
    ...
    def _upload_and_push_file(self, filepath, body=None, deviceid=None, apikey=None): 
        try:
            ...
            logger.error("Could not send file to Pushbullet notification. Error: {0}".format(e))
    ...
    def _push(self, data, deviceid=None, apikey=None, file={}):
        ...
        try:
            response = requests.post(self._apiurl, data=data, headers=headers, files=file, auth=apikey)
            if self._is_response_ok(response) == False:
                logger.error("Could not send Pushbullet notification. Error: {0}".format(response.text))
        except Exception as e:
            logger.error("Could not send Pushbullet notification. Error: {0}".format(e))
