...
### Visu Elements
You could find a example config with the supported elements in the examples directory (examples/items/visu.conf).
...
### Step by Step
   ...
   * set the the generator_dir in the plugin.conf to '/var/www/visu'
   ...
   * set the visu attribute in the items configuration to an element.
   * start SmartHome.py
   * goto http://yourserver/dyn/
