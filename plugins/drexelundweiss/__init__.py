        ...
        for register in self.LUregl:
            ...
            for item in self.LUregl[register]['items']:
                ...
                if done:
                    ...
                    logger.debug("Drexel: Init LU register: "+str(register)+" failed!")
        ...
        for register in self.WPregl:
            ...
            for item in self.WPregl[register]['items']:
                ...
                if done:
                    ...
                    logger.debug("Drexel: Init WP register: "+str(register)+" failed!")
                            ...
                            if (response[-1] == 0x20 and dw_id == 0):
                                ...
                                if (self.busmonitor):
                                    if dw_id == self._LU_ID:
                                        if dw_register in self.LUcmdl:
                                            ...
                                            divisor=int(reginfo[4])
                                            komma=int(reginfo[5])
                                            logger.debug("DuW Busmonitor LU register: "+str(dw_register)+" "+str(reginfo[1])+": "+str(((dw_data/divisor)/(10**komma))))
                                        else:
                                            logger.debug("DuW Busmonitor: unknown LU register: "+str(dw_register)+" "+str(dw_data))
                                    elif dw_id == self._WP_ID:
                                        if dw_register in self.WPcmdl:
                                            reginfo=self.WPcmdl[dw_register]['reginfo']                                      
                                            divisor=int(reginfo[4])
                                            komma=int(reginfo[5])
                                            logger.debug("DuW Busmonitor WP register: "+str(dw_register)+" "+str(reginfo[1])+": "+str(((dw_data/divisor)/(10**komma))))
                                        else:
                                            logger.debug("DuW Busmonitor: unknown WP register: "+str(dw_register)+" "+str(dw_data))
                                    else:
                                        logger.debug("DuW Busmonitor: unknown device ID: "+str(dw_id))
                                ...
                                if dw_id == self._LU_ID: 
                                    if dw_register in self.LUregl:
                                        ...
                                        logger.debug("Ignore LU register "+str(dw_register))
                                elif dw_id == self._WP_ID:
                                    if dw_register in self.WPregl:
                                        ...
                                        logger.debug("Ignore WP register "+str(dw_register))
                                else:
                                    logger.debug("DuW Busmonitor: unknown device ID: "+str(dw_id))
    ...
    def parse_item(self, item):
        ...
        if 'DuW_LU_register' in item.conf:
            ...
            if reginfo:              
                ...
                logger.warning("Drexel: LU register: "+str(register) +" not supported by configured device!")
                return None
        if 'DuW_WP_register' in item.conf:
            ...
            if reginfo:              
                ...
                logger.warning("Drexel: WP register: "+str(register) +" not supported by configured device!")
                return None
        else:
            return None
            ...
            if 'DuW_LU_register' in item.conf:
                ...
                if register in self.LUregl:
                    ...
                    if (data<int(reginfo[2]) or data>int(reginfo[3])):
                        logger.error("DuW: value of LU register: "+str(register) +" out of range, changes ignored!")
                        pass
                    else:
                        if reginfo[6]=='R/W':
                            logger.debug("DuW: update LU register: "+str(register)+" "+reginfo[1]+" with: "+str(data))
                            self.write_DW(reginfo[7],register, data)
                        else:
                            logger.warning("DuW: tried to update read only LU register: "+str(register))
            if 'DuW_WP_register' in item.conf:
                register=int(item.conf['DuW_WP_register'])
                if register in self.WPregl:
                    ...
                    data=item()*int(reginfo[4])*(10**int(reginfo[5]))
                    if (data<int(reginfo[2]) or data>int(reginfo[3])):
                        logger.error("DuW: value of WP register: "+str(register) +" out of range, changes ignored!")
                        pass
                    else:
                        if reginfo[6]=='R/W':
                            logger.debug("DuW: update LU register: "+str(register)+" "+reginfo[1]+" with: "+str(data))
                            ...
                            logger.warning("DuW: tried to update read only WP register: "+str(register))