...
class SmartHome():
    ...
    def __init__(self, smarthome_conf='/usr/local/smarthome/etc/smarthome.conf'):
        ...
        self.log = lib.log.Log(self, 'SmartHome.py', '<p style="font-weight:bold;">{1}</p><p>{3}</p><p class="ui-li-aside">{0:%H:%M:%S}<br />{2}</p>', maxlen=self._log_buffer)
        ...
        if self._connections != []:
            ...
            self.scheduler.add('sh.con', self._connection_monitor, cycle=60, offset=0)
        self._plugins.start()
